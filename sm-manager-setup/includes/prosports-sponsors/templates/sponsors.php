...
$defaults = array(
	...
	'orderby' => 'menu_order',
	'order' => 'ASC',
	'size' => 'prosports-fit-icon',
);

extract( $defaults, EXTR_SKIP );

if ( $size == 'prosports-fit-icon' && (int) $height > 128 || (int) $width > 128 )
	$size = 'prosports-fit-medium';
	...
	<div class="sp-sponsors">
		...
		<?php foreach( $sponsors as $sponsor ): ?>
			<?php if ( $sponsor->post_content == '' ): $url = get_post_meta( $sponsor->ID, 'sp_url', true ); ?>
				<a class="sponsor sp-sponsor"<?php if ( $url ): ?> href="<?php echo $url; ?>" data-nonce="<?php echo wp_create_nonce( 'sp_clicks_' . $sponsor->ID ); ?>" data-ajaxurl="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-post="<?php echo $sponsor->ID; ?>"<?php endif; if ( get_option( 'prosports_sponsor_site_target_blank', 'no' ) == 'yes' ) { ?> target="_blank"<?php } ?>>
			<?php else: ?>
