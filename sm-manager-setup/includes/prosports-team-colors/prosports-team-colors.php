<?php
/*
Plugin Name: ProSports Team Colors
Plugin URI: http://prosportspro.com/
Description: Add team colors to ProSports.
Author: ProSports
...
/**
 * Main ProSports Team Colors Class
 *
 * @class ProSports_Team_Colors
 * @version	1.0
 */
class ProSports_Team_Colors {
	...
	public function __construct() {
		...
		// Hooks
		add_filter( 'option_prosports_frontend_css_colors', array( $this, 'css' ) );
		add_action( 'add_meta_boxes', array( $this, 'add_meta_boxes' ) );
		add_action( 'prosports_process_sp_team_meta', array( $this, 'meta_box_save' ), 15, 2 );
	...
	*/
	...
	public function add_meta_boxes() {
		add_meta_box( 'sp_colorssdiv', __( 'Team Colors', 'prosports' ), array( $this, 'meta_box_output' ), 'sp_team', 'normal', 'high' );
	}
	...
	public function meta_box_output( $post ) {
		...
		// Global settings
		$options = array_map( 'esc_attr', (array) get_option( 'prosports_frontend_css_colors', array() ) );
		...
		foreach( $options as $name => $color ) {
			...
			$this->color_picker( __( ucwords( $name ), 'prosports' ), $name, $value, $enabled );
		}
		?>
		<div class="clear"></div>
		<?php
