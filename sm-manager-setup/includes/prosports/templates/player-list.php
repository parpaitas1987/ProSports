...
$defaults = array(
	...
	'show_player_photo' => get_option( 'prosports_list_show_photos', 'yes' ) == 'yes' ? true : false,
	...
	'paginated' => get_option( 'prosports_list_paginated', 'yes' ) == 'yes' ? true : false,
	'rows' => get_option( 'prosports_list_rows', 10 ),
);
...
foreach ( $groups as $group ):
	...
	if ( ! is_array( $labels ) || array_key_exists( 'number', $labels ) ):
		if ( in_array( $orderby, array( 'number', 'name' ) ) ):
			...
			$output .= '<th class="data-rank">' . __( 'Rank', 'prosports' ) . '</th>';
		endif;
	endif;
	...
	foreach( $data as $player_id => $row ): if ( empty( $group->term_id ) || has_term( $group->term_id, 'sp_position', $player_id ) ):
		...
		if ( $show_player_photo ):
			if ( has_post_thumbnail( $player_id ) ):
				$logo = get_the_post_thumbnail( $player_id, 'prosports-fit-icon' );
...
if ( $show_all_players_link )
	$output .= '<a class="sp-player-list-link sp-view-all-link" href="' . get_permalink( $id ) . '">' . __( 'View all players', 'prosports' ) . '</a>';
?>
