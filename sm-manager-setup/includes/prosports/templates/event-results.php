...
if ( get_option( 'prosports_event_show_results', 'yes' ) === 'no' ) return;
...
if ( empty( $table_rows ) ):
	...
	$output .= '<h4 class="sp-table-caption">' . __( 'Team Results', 'prosports' ) . '</h4>';
	...
	$output .= '<div class="sp-table-wrapper' . ( $scrollable ? ' sp-scrollable-table-wrapper' : '' ) . '">' .
		...
		'<th class="data-name">' . __( 'Team', 'prosports' ) . '</th>';
	foreach( $labels as $key => $label ):
