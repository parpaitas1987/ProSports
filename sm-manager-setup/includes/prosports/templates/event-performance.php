...
$show_players = get_option( 'prosports_event_show_players', 'yes' ) === 'yes' ? true : false;
$show_staff = get_option( 'prosports_event_show_staff', 'yes' ) === 'yes' ? true : false;
$show_total = get_option( 'prosports_event_show_total', 'yes' ) === 'yes' ? true : false;
...
if ( is_array( $teams ) ):
	...
	$link_posts = get_option( 'prosports_link_players', 'yes' ) == 'yes' ? true : false;
	...
	$mode = get_option( 'prosports_event_performance_mode', 'values' );
						...
						<thead>
							<tr>
								<?php if ( $show_team_players ): ?>
									...
									<th class="data-name"><?php _e( 'Player', 'prosports' ); ?></th>
								<?php endif; ?>
									...
									foreach( $labels as $key => $label ):
										...
										elseif ( intval( $value ) && $mode == 'icons' ):
											...
											if ( $performance_id && has_post_thumbnail( $performance_id ) ):
												echo str_repeat( get_the_post_thumbnail( $performance_id, 'prosports-fit-mini' ) . ' ', $value );
											endif;
										endif;
									endforeach;
							...
							<<?php echo ( $show_team_players ? 'tfoot' : 'tbody' ); ?>>
								<tr class="' . ( $i % 2 == 0 ? 'odd' : 'even' ) . '">
									...
									if ( $show_team_players ):
										...
										echo '<td class="data-name">' . __( 'Total', 'prosports' ) . '</td>';
									endif;
