...
$defaults = array(
	...
	'show_team_logo' => get_option( 'prosports_table_show_logos', 'yes' ) == 'yes' ? true : false,
	'link_posts' => get_option( 'prosports_link_teams', 'no' ) == 'yes' ? true : false,
	...
	'paginated' => get_option( 'prosports_table_paginated', 'yes' ) == 'yes' ? true : false,
	'rows' => get_option( 'prosports_table_rows', 10 ),
);
...
$output .= '<th class="data-rank">' . __( 'Pos', 'prosports' ) . '</th>';
...
foreach ( $data as $team_id => $row ):
	...
	if ( $show_team_logo ):
		if ( has_post_thumbnail( $team_id ) ):
			$logo = get_the_post_thumbnail( $team_id, 'prosports-fit-icon' );
...
if ( $show_full_table_link )
	$output .= '<a class="sp-league-table-link sp-view-all-link" href="' . get_permalink( $id ) . '">' . __( 'View full table', 'prosports' ) . '</a>';
