...
$primary_result = get_option( 'prosports_primary_result', null );
...
$defaults = array(
	...
	'paginated' => get_option( 'prosports_event_blocks_paginated', 'yes' ) == 'yes' ? true : false,
	'rows' => get_option( 'prosports_event_blocks_rows', 10 ),
	'order' => 'default',
	'show_all_events_link' => false,
	'show_league' => get_option( 'prosports_event_blocks_show_league', 'no' ) == 'yes' ? true : false,
	'show_season' => get_option( 'prosports_event_blocks_show_season', 'no' ) == 'yes' ? true : false,
	'show_venue' => get_option( 'prosports_event_blocks_show_venue', 'no' ) == 'yes' ? true : false,
);
				...
				foreach ( $data as $event ):
					...
					foreach( $teams as $team ):
						...
						if ( has_post_thumbnail ( $team ) ):
							if ( $link_teams ):
								$logo = '<a class="team-logo logo-' . ( $j % 2 ? 'odd' : 'even' ) . '" href="' . get_post_permalink( $team ) . '" title="' . get_the_title( $team ) . '">' . get_the_post_thumbnail( $team, 'prosports-fit-icon' ) . '</a>';
							else:
								$logo = get_the_post_thumbnail( $team, 'prosports-fit-icon', array( 'class' => 'team-logo logo-' . ( $j % 2 ? 'odd' : 'even' ) ) );
							endif;
							$logos[] = $logo;
						endif;
