...
function sp_get_template_part( $slug, $name = '' ) {
	...
	// Look in yourtheme/slug-name.php and yourtheme/prosports/slug-name.php
	if ( $name ) {
	...
	// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/prosports/slug.php
	if ( ! $template ) {
		$template = locate_template( array( "{$slug}.php", SP()->template_path() . "{$slug}.php" ) );
	}

	// Allow 3rd party plugin filter template file from their plugin
	$template = apply_filters( 'prosports_get_template_part', $template, $slug, $name );
...
function sp_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	...
	do_action( 'prosports_before_template_part', $template_name, $template_path, $located, $args );

	include( $located );

	do_action( 'prosports_after_template_part', $template_name, $template_path, $located, $args );
}
...
function sp_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	...
	return apply_filters('prosports_locate_template', $template, $template_name, $template_path);
}
...
if ( !function_exists( 'sp_get_url' ) ) {
	function sp_get_url( $post_id ) {
		...
		if ( ! $url ) return;
		return ' <a class="sp-link" href="' . $url . '" target="_blank" title="' . __( 'Visit Site', 'prosports' ) . '">' . $url . '</a>';
	}
}
...
if ( !function_exists( 'sp_get_post_calculate' ) ) {
	function sp_get_post_calculate( $post_id ) {
		...
		if ( $calculate ):
			return str_replace(
				array( 'total', 'average' ),
				array( __( 'Total', 'prosports' ), __( 'Average', 'prosports' ) ),
				$calculate
			);
		else:
			return __( 'Total', 'prosports' );
		endif;
	}
}
...
if ( !function_exists( 'sp_dropdown_statuses' ) ) {
	function sp_dropdown_statuses( $args = array() ) {
		...
		$statuses = apply_filters( 'prosports_statuses', array(
			'any' => __( 'All', 'prosports' ),
			'publish' => __( 'Published', 'prosports' ),
			'future' => __( 'Scheduled', 'prosports' )
		));
...
if ( !function_exists( 'sp_dropdown_dates' ) ) {
	function sp_dropdown_dates( $args = array() ) {
		...
		$dates = apply_filters( 'prosports_dates', array(
			0 => __( 'All', 'prosports' ),
			'w' => __( 'This week', 'prosports' ),
			'day' => __( 'Today', 'prosports' ),
			'range' => __( 'Date range:', 'prosports' ),
		));
	...
	function sp_post_checklist( $post_id = null, $meta = 'post', $display = 'block', $filters = null, $index = null ) {
		...
		<div id="<?php echo $meta; ?>-all" class="posttypediv wp-tab-panel sp-tab-panel sp-select-all-range" style="display: <?php echo $display; ?>;">
			...
			<ul class="categorychecklist form-no-clear">
				<li class="sp-select-all-container"><label class="selectit"><input type="checkbox" class="sp-select-all"> <strong><?php _e( 'Select All', 'prosports' ); ?></strong></label></li>
				<?php
				...
				<li class="sp-not-found-container">
					<?php _e( 'No results found.', 'prosports' ); ?>
					<?php if ( sizeof( $posts ) ): ?><a class="sp-show-all" href="#show-all-<?php echo $meta; ?>s"><?php _e( 'Show all', 'prosports' ); ?></a><?php endif; ?>
				</li>
				<?php if ( sizeof( $posts ) ): ?>
					<li class="sp-show-all-container"><a class="sp-show-all" href="#show-all-<?php echo $meta; ?>s"><?php _e( 'Show all', 'prosports' ); ?></a></li>
				<?php endif; ?>
			</ul>
		</div>
		<?php
	...
	function sp_column_checklist( $post_id = null, $meta = 'post', $display = 'block', $selected = array(), $default_checked = false ) {
		...
		<div id="<?php echo $meta; ?>-all" class="posttypediv wp-tab-panel sp-tab-panel sp-select-all-range" style="display: <?php echo $display; ?>;">
			...
			<ul class="categorychecklist form-no-clear">
				...
				?>
				<li class="sp-not-found-container"><?php _e( 'No results found.', 'prosports' ); ?></li>
				<?php endif; ?>
			</ul>
		</div>
		<?php
...
if ( !function_exists( 'sp_sort_table_teams' ) ) {
	function sp_sort_table_teams ( $a, $b ) {
		...
		global $prosports_column_priorities;

		// Loop through priorities
		foreach( $prosports_column_priorities as $priority ):
...
function sp_get_text_options() {
	$strings = apply_filters( 'prosports_text', array(
		__( 'Article', 'prosports' ),
		__( 'Career Total', 'prosports' ),
		__( 'Current Team', 'prosports' ),
		__( 'Current Teams', 'prosports' ),
		__( 'Date', 'prosports' ),
		__( 'Details', 'prosports' ),
		__( 'Event', 'prosports' ),
		__( 'Competition', 'prosports' ),
		__( 'Nationality', 'prosports' ),
		__( 'Outcome', 'prosports' ),
		__( 'Past Teams', 'prosports' ),
		__( 'Played', 'prosports' ),
		__( 'Player', 'prosports' ),
		__( 'Pos', 'prosports' ),
		__( 'Position', 'prosports' ),
		__( 'Preview', 'prosports' ),
		__( 'Rank', 'prosports' ),
		__( 'Recap', 'prosports' ),
		__( 'Team Results', 'prosports' ),
		__( 'Season', 'prosports' ),
		__( 'Staff', 'prosports' ),
		__( 'Substitutes', 'prosports' ),
		__( 'Team', 'prosports' ),
		__( 'Teams', 'prosports' ),
		__( 'Time', 'prosports' ),
		__( 'Time/Results', 'prosports' ),
		__( 'Total', 'prosports' ),
		__( 'Venue', 'prosports' ),
		__( 'Video', 'prosports' ),
		__( 'View all events', 'prosports' ),
		__( 'View all players', 'prosports' ),
		__( 'View full table', 'prosports' ),
	));
...
function sp_review_link() {
	?>
	<p>
		<a href="http://wordpress.org/support/view/plugin-reviews/prosports?rate=5#postform">
			<?php _e( 'Love ProSports? Help spread the word by rating us 5★ on WordPress.org', 'prosports' ); ?>
		</a>
	</p>
	<?php
}
