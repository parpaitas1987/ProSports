...
class SP_Install {
	...
	public function __construct() {
		...
		add_action( 'in_plugin_update_message-prosports/prosports.php', array( $this, 'in_plugin_update_message' ) );
	}
	...
	public function check_version() {
		if ( ! defined( 'IFRAME_REQUEST' ) && get_option( 'prosports_version' ) != SP()->version ) {
			$this->install();

			do_action( 'prosports_updated' );
		}
	}
	...
	public function install_actions() {
		...
		if ( ! empty( $_GET['install_prosports'] ) ) {
		...
		// Skip button
		} elseif ( ! empty( $_GET['skip_install_prosports'] ) ) {
	...
	public function install() {
		...
		// Queue upgrades
		$current_version = get_option( 'prosports_version', null );

		// Update version
		update_option( 'prosports_version', SP()->version );

		// Check if pages are needed
		if ( ! get_option( 'prosports_sport' ) ) {
	...
	function create_options() {
	    ...
	    add_option( 'prosports_frontend_css_primary', '#2b353e' );
	    add_option( 'prosports_frontend_css_background', '#f4f4f4' );
	    add_option( 'prosports_frontend_css_text', '#222222' );
	    add_option( 'prosports_frontend_css_heading', '#ffffff' );
	    add_option( 'prosports_frontend_css_link', '#00a69c' );

		if ( ! get_option( 'prosports_installed' ) ) {
			// Configure default sport
			$sport = 'custom';
		    update_option( 'prosports_sport', $sport );

			// Flag as installed
			update_option( 'prosports_installed', 1 );
		}
	}
	...
	/**
	 * Get capabilities for ProSports - these are assigned during installation or reset
	 *
	 * @access public
	 * @return array
	 */
	public function get_core_capabilities() {
		...
		$capabilities['core'] = array(
			'manage_prosports',
			'view_prosports_reports',
		);

		$post_types = sp_post_types();
		array_unshift( $post_types, 'sp_config' );

		$capability_types = apply_filters( 'prosports_post_types', $post_types );
	...
	/**
	 * prosports_remove_roles function.
	 *
	 * @access public
	 * @return void
	 */
	...
	function in_plugin_update_message() {
		$response = wp_remote_get( 'https://plugins.svn.wordpress.org/prosports/trunk/readme.txt' );
		...
		if ( ! is_wp_error( $response ) && ! empty( $response['body'] ) ) {
			...
			if ( preg_match( $regexp, $response['body'], $matches ) ) {
				...
				_e( 'What\'s new:', 'prosports' ) . '<div style="font-weight: normal;">';
