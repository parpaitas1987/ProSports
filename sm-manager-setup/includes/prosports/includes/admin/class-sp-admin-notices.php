...
class SP_Admin_Notices {
	...
	public function __construct() {
		...
		add_action( 'prosports_updated', array( $this, 'reset_admin_notices' ) );
	...
	public function reset_admin_notices() {
		update_option( 'prosports_admin_notices', array( 'template_files', 'theme_support' ) );
	}
	...
	public function add_notices() {
		...
		$notices = get_option( 'prosports_admin_notices', array() );

		if ( get_option( '_sp_needs_welcome' ) == 1 && $screen->id != 'toplevel_page_prosports' ) {
			wp_enqueue_style( 'prosports-activation', plugins_url(  '/assets/css/activation.css', SP_PLUGIN_FILE ) );
			add_action( 'admin_notices', array( $this, 'install_notice' ) );
		}

		if ( ! empty( $_GET['hide_theme_support_notice'] ) ) {
			$notices = array_diff( $notices, array( 'theme_support' ) );
			update_option( 'prosports_admin_notices', $notices );
		}
		...
		if ( in_array( 'theme_support', $notices ) && ! current_theme_supports( 'prosports' ) ) {
	...
	public function template_file_check_notice() {
		if ( isset( $_GET['page'] ) && 'prosports' == $_GET['page'] && isset( $_GET['tab'] ) && 'status' == $_GET['tab'] ) {
			return;
		}
