<?php
/**
 * Player importer - import players into ProSports.
 *
 * @author 		ProSports
 * @category 	Admin
...
if ( class_exists( 'WP_Importer' ) ) {
	class SP_Player_Importer extends SP_Importer {
		...
		public function __construct() {
			$this->import_page = 'prosports_player_csv';
			$this->import_label = __( 'Import Players', 'prosports' );
			$this->columns = array(
				'sp_number' => __( 'Number', 'prosports' ),
				'post_title' => __( 'Name', 'prosports' ),
				'sp_position' => __( 'Positions', 'prosports' ),
		...
		function import( $array = array(), $columns = array( 'post_title' ) ) {
			...
			echo '<div class="updated settings-error below-h2"><p>
				'.sprintf( __( 'Import complete - imported <strong>%s</strong> players and skipped <strong>%s</strong>.', 'prosports' ), $this->imported, $this->skipped ).'
			</p></div>';
		 ...
		 */
		function import_end() {
			echo '<p>' . __( 'All done!', 'prosports' ) . ' <a href="' . admin_url('edit.php?post_type=sp_player') . '">' . __( 'View Players', 'prosports' ) . '</a>' . '</p>';
		 ...
		 */
		function header() {
			echo '<div class="wrap"><h2>' . __( 'Import Players', 'prosports' ) . '</h2>';
		}
		...
		function greet() {
			...
			echo '<p>' . sprintf( __( 'Players need to be defined with columns in a specific order (7 columns). <a href="%s">Click here to download a sample</a>.', 'prosports' ), plugin_dir_url( SP_PLUGIN_FILE ) . 'dummy-data/players-sample.csv' ) . '</p>';
			wp_import_upload_form( 'admin.php?import=prosports_player_csv&step=1' );
			echo '</div>';
		}
	}
}
