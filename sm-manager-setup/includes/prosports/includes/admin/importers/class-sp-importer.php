...
if ( class_exists( 'WP_Importer' ) ) {
	class SP_Importer extends WP_Importer {
		...
		public function admin_scripts() {
		    wp_enqueue_script( 'prosports-admin', SP()->plugin_url() . '/assets/js/admin/prosports-admin.js', array( 'jquery', 'chosen', 'jquery-ui-core', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'jquery-tiptip', 'jquery-caret', 'jquery-countdown' ), SP_VERSION, true );
		}
		...
		function dropdown( $selected ) {
			...
			<select name="sp_columns[]" data-index="<?php echo array_search( $selected, array_keys( $this->columns ) ); ?>">
				<option value="0">&mdash; <?php _e( 'Disable', 'prosports' ); ?> &mdash;</option>
		...
		function table( $file ) {
			...
			if ( ( $handle = fopen( $file, "r" ) ) !== FALSE ):
				...
				if ( sizeof( $header ) >= 1 ):
					...
					?>
					<form enctype="multipart/form-data" id="import-upload-form" class="prosports" method="post" action="<?php echo esc_attr(wp_nonce_url($action, 'import-upload')); ?>">
						<?php $this->options(); ?>
							...
							<thead>
								<tr>
									...
									<th scope="col" class="sp-actions-column">
										<a href="#" title="<?php _e( 'Insert row after', 'prosports' ); ?>" class="dashicons dashicons-plus-alt sp-add-row sp-add-first"></a>
									</th>
								</tr>
							</thead>
							<tbody>
								<?php while ( ( $row = fgetcsv( $handle, 0, $this->delimiter ) ) !== FALSE ): ?>
									<tr>
										...
										<td class="sp-actions-column">
											<a href="#" title="<?php _e( 'Delete row', 'prosports' ); ?>" class="sp-icon-trash sp-delete-row"></a>
											<a href="#" title="<?php _e( 'Insert row after', 'prosports' ); ?>" class="dashicons dashicons-plus-alt sp-add-row"></a>
										</td>
									</tr>
						...
						<p class="sp-post-count alignright">
							<?php printf( __( 'Displaying %s&#8211;%s of %s', 'prosports' ), 1, $this->imported+1, $this->imported+1 ); ?>
						</p>
						<p class="submit">
					...
					echo '<p><strong>' . __( 'Sorry, there has been an error.', 'prosports' ) . '</strong><br />';
					_e( 'The CSV is invalid.', 'prosports' ) . '</p>';
					$this->footer();
					die();
		...
		function handle_upload() {
			...
			if ( empty( $_POST['file_url'] ) ) {
				...
				if ( file_exists( ABSPATH . $_POST['file_url'] ) ) {
					...
					echo '<p><strong>' . __( 'Sorry, there has been an error.', 'prosports' ) . '</strong></p>';
					return false;
