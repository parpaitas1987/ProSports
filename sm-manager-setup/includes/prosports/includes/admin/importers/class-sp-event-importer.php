<?php
/**
 * Event importer - import events into ProSports.
 *
 * @author 		ProSports
 * @category 	Admin
...
if ( class_exists( 'WP_Importer' ) ) {
	class SP_Event_Importer extends SP_Importer {
		...
		public function __construct() {
			$this->import_page = 'prosports_event_csv';
			$this->import_label = __( 'Import Events', 'prosports' );
			$this->columns = array(
				'post_date' => __( 'Date', 'prosports' ),
				'post_time' => __( 'Time', 'prosports' ),
				'sp_venue' => __( 'Venue', 'prosports' ),
				'sp_team' => __( 'Teams', 'prosports' ),
				'sp_results' => __( 'Results', 'prosports' ),
				'sp_outcome' => __( 'Outcome', 'prosports' ),
				'sp_player' => __( 'Players', 'prosports' ),
			);
		...
		function import( $array = array(), $columns = array( 'post_title' ) ) {
			...
			foreach ( $rows as $row ):
				...
				if ( sizeof( $team ) > 0 && ! empty( $team[0] ) ):
					...
					if ( isset( $id ) ):
						...
						if ( $title ):
							$title .= ' ' . get_option( 'prosports_event_teams_delimiter', 'vs' ) . ' ';
						endif;
			...
			echo '<div class="updated settings-error below-h2"><p>
				'.sprintf( __( 'Import complete - imported <strong>%s</strong> events and skipped <strong>%s</strong>.', 'prosports' ), $this->imported, $this->skipped ).'
			</p></div>';
		 ...
		 */
		function import_end() {
			echo '<p>' . __( 'All done!', 'prosports' ) . ' <a href="' . admin_url('edit.php?post_type=sp_event') . '">' . __( 'View Events', 'prosports' ) . '</a>' . '</p>';
		...
		function greet() {
			...
			echo '<p>' . sprintf( __( 'Events need to be defined with columns in a specific order (3+ columns). <a href="%s">Click here to download a sample</a>.', 'prosports' ), plugin_dir_url( SP_PLUGIN_FILE ) . 'dummy-data/events-sample.csv' ) . '</p>';
			wp_import_upload_form( 'admin.php?import=prosports_event_csv&step=1' );
			echo '</div>';
		}
		...
		function options() {
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th scope="row"><label><?php _e( 'Format', 'prosports' ); ?></label><br/></th>
						<td class="forminp forminp-radio" id="sp_formatdiv">
							<fieldset id="post-formats-select">
								<ul>
									<li><input type="radio" name="sp_format" class="post-format" id="post-format-league" value="league" checked="checked"> <label for="post-format-league" class="post-format-icon post-format-league"><?php _e( 'Competitive', 'prosports' ); ?></label></li>
									<li><input type="radio" name="sp_format" class="post-format" id="post-format-friendly" value="friendly"> <label for="post-format-friendly" class="post-format-icon post-format-friendly"><?php _e( 'Friendly', 'prosports' ); ?></label></li>
								<br>
							</fieldset>
						</td>
					</tr>
					<tr>
						<th scope="row"><label><?php _e( 'Competition', 'prosports' ); ?></label><br/></th>
						<td><?php
						$args = array(
							'taxonomy' => 'sp_league',
							'name' => 'sp_league',
							'values' => 'slug',
							'show_option_none' => __( '-- Not set --', 'prosports' ),
						);
						if ( ! sp_dropdown_taxonomies( $args ) ):
							echo '<p>' . __( 'None', 'prosports' ) . '</p>';
							sp_taxonomy_adder( 'sp_league', 'sp_team', __( 'Add New', 'prosports' ) );
						endif;
						?></td>
					</tr>
					<tr>
						<th scope="row"><label><?php _e( 'Season', 'prosports' ); ?></label><br/></th>
						<td><?php
						$args = array(
							'taxonomy' => 'sp_season',
							'name' => 'sp_season',
