<div class="wrap prosports prosports-overview-wrap">
	<h2>
		<?php _e( 'Overview', 'prosports' ); ?>
	</h2>
	<div class="sp-sitemap">
		...
		<?php if ( ! isset( $_GET['type'] ) && ! isset( $_GET['taxonomy'] ) ): // Overview ?>
			...
			<ul class="sp-utility">
				<?php foreach ( $primary_post_types as $post_type ): $object = get_post_type_object( $post_type ); ?>
					<li><a class="button button-primary" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'type' => $post_type ), 'admin.php' ) ) ); ?>"><?php echo $object->labels->name; ?></a></li>
				<?php endforeach; ?>
			</ul>

			<?php $taxonomies = sp_taxonomies(); ?>
			<ul class="sp-primary col<?php echo sizeof( $taxonomies ); ?>">
				<li class="sp-home"><a class="button disabled"><?php _e( 'ProSports', 'prosports' ); ?></a></li>
				<?php foreach ( $taxonomies as $taxonomy ): $object = get_taxonomy( $taxonomy ); $post_types = apply_filters( 'prosports_sitemap_taxonomy_post_types', $object->object_type, $taxonomy ); ?>
					<li><a class="button button-primary" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy ), 'admin.php' ) ) ); ?>"><?php echo $object->labels->name; ?></a>
							...
							<?php if ( sizeof( $terms ) > 0 ): ?>
								<?php foreach ( $terms as $term ): ?>
									...
									<li>
										...
										<?php else: ?>
											<a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $term->term_id ), 'admin.php' ) ) ); ?>"><?php echo wp_strip_all_tags( $term->name ); ?></a>
										<?php endif; ?>
												...
												<?php foreach ( $children as $child ): ?>
													<li>
														<?php if ( sizeof( $post_types ) <= 1 ): ?>
															...
															<?php endforeach; endif; ?>
														<?php else: ?>
															<a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $child->term_id ), 'admin.php' ) ) ); ?>"><?php echo wp_strip_all_tags( $child->name ); ?></a>
														<?php endif; ?>
													</li>
												<?php endforeach; ?>
											</ul>
		...
		<?php elseif ( ! isset( $_GET['type'] ) ): $taxonomy = $_GET['taxonomy']; $taxonomy_object = get_taxonomy( $taxonomy ); ?>
			...
			<?php $post_types = apply_filters( 'prosports_sitemap_taxonomy_post_types', $taxonomy_object->object_type, $taxonomy ); ?>
			...
			<?php if ( isset( $_GET['term'] ) ): $term = get_term( $_GET['term'], $taxonomy ); // Posts in term ?>
				...
				<?php if ( $terms ): // Has children ?>
					...
					<ul class="sp-primary col<?php echo sizeof( $terms ) + 1; ?>">
						<li class="sp-breadcrumb"><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview' ), 'admin.php' ) ) ); ?>"><?php _e( 'ProSports', 'prosports' ); ?></a></li>
						<li class="sp-breadcrumb"><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy ), 'admin.php' ) ) ); ?>"><?php echo $taxonomy_object->labels->name; ?></a></li>
						<li class="sp-home"><a class="button disabled"><?php echo wp_strip_all_tags( $term->name ); ?></a></li>
						<li><a class="button disabled"><?php _e( 'All', 'prosports' ); ?></a>
							<ul>
						...
						<?php foreach ( $terms as $term ): ?>
							<li><a class="button button-primary" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $term->term_id ), 'admin.php' ) ) ); ?>"><?php echo wp_strip_all_tags( $term->name ); ?></a>
								<?php $children = get_terms( $taxonomy, array( 'hide_empty' => false, 'parent' => $term->term_id, 'orderby' => 'slug' ) ); ?>
				...
				<?php else: // No children ?>
					...
					<ul class="sp-primary col<?php echo sizeof( $post_types ); ?>">
						...
						<?php if ( $term->parent ): $parent = get_term( $term->parent, $taxonomy ); ?>
							<li class="sp-breadcrumb"><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $term->parent ), 'admin.php' ) ) ); ?>"><?php echo $parent->name; ?></a></li>
						<?php endif; ?>
						<li class="sp-home"><a class="button disabled"><?php echo wp_strip_all_tags( $term->name ); ?></a></li>
					...
					<?php if ( $terms ): ?>
						<?php foreach ( $terms as $term ): ?>
							<li>
								...
								<?php else: ?>
									<a class="button button-primary" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $term->term_id ), 'admin.php' ) ) ); ?>"><?php echo wp_strip_all_tags( $term->name ); ?></a>
								<?php endif; ?>
								<?php $children = get_terms( $taxonomy, array( 'hide_empty' => false, 'parent' => $term->term_id, 'orderby' => 'slug' ) ); ?>
								<ul>
									...
									<?php if ( $children ): // Has children ?>
										...
										<?php else: ?>
											...
											<?php foreach ( $children as $child ): ?>
												<li><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'taxonomy' => $taxonomy, 'term' => $child->term_id ), 'admin.php' ) ) ); ?>"><?php echo wp_strip_all_tags( $child->name ); ?></a>
													<?php if ( sizeof ( $post_types ) ): ?>
														<ul>
															<?php foreach ( $post_types as $post_type ): $post_object = get_post_type_object( $post_type ); ?>
																<li><a class="button action" href="<?php echo esc_url( admin_url( add_query_arg( array( 'post_type' => $post_type, $taxonomy => $child->slug ), 'edit.php' ) ) ); ?>"><?php echo $post_object->labels->name; ?><span class="dashicons dashicons-list-view wp-ui-text-highlight"></span></a></li>
		...
		<?php elseif ( ! isset( $_GET['taxonomy'] ) ): $post_type = $_GET['type']; // Post type archive ?>
			...
			<ul class="sp-utility">
				...
				<li><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'post_type' => $post_type ), 'edit.php' ) ) ); ?>"><?php _e( 'View All', 'prosports' ); ?></a></li>
			</ul>
			...
			<?php if ( sizeof( $taxonomies ) || sizeof( sp_array_value( $hierarchy, $post_type ) ) ): // Display taxonomies ?>
				<ul class="sp-primary col<?php echo sizeof( $taxonomies ) + sizeof( sp_array_value( $hierarchy, $post_type ) ); ?>">
					...
					<?php foreach ( $taxonomies as $taxonomy_object ): $taxonomy = $taxonomy_object->name; ?>
						<li><a class="button button-primary" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'type' => $post_type, 'taxonomy' => $taxonomy_object->name ), 'admin.php' ) ) ); ?>"><?php echo $taxonomy_object->labels->name; ?></a>
							<?php $terms = get_terms( $taxonomy, array( 'hide_empty' => false, 'parent' => 0, 'orderby' => 'slug' ) ); ?>
							<?php if ( $terms ): ?>
								<ul>
									<?php foreach ( $terms as $term ): ?>
			...
			<?php else: // Display posts ?>
				...
				<ul class="sp-primary col<?php echo sizeof( $posts ); ?>">
					...
					<?php if ( $posts ): ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<li><a class="button disabled"><?php _e( 'No results found.', 'prosports' ); ?></a></li>
					<?php endif; ?>
				</ul>
		...
		<?php else: $post_type = $_GET['type']; $taxonomy = $_GET['taxonomy']; // Filtered posts ?>
			...
			<ul class="sp-primary col<?php echo sizeof( $terms ) + 1; ?>">
				<li class="sp-breadcrumb"><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview' ), 'admin.php' ) ) ); ?>"><?php _e( 'ProSports', 'prosports' ); ?></a></li>
				<li class="sp-breadcrumb"><a class="button" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'prosports-overview', 'type' => $post_type ), 'admin.php' ) ) ); ?>"><?php echo $post_object->labels->name; ?></a></li>
				<li class="sp-home"><a class="button disabled"><?php echo $taxonomy_object->labels->name; ?></a></li>
	...
	<?php do_action( 'prosports_overview_page' ); ?>
</div>