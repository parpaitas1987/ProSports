...
class SP_Admin_Dashboard {
    ...
    public function __construct() {
        ...
        if ( current_user_can( 'view_prosports_reports' ) || current_user_can( 'manage_prosports' ) || current_user_can( 'publish_sp_events' ) ) {
    ...
    public function init() {
        wp_add_dashboard_widget( 'prosports_dashboard_status', __( 'ProSports', 'prosports' ), array( $this, 'status_widget' ) );
    ...
    function glance_items( $items = array() ) {
        ...
        foreach ( $post_types as $type ):
            ...
            if ( $num_posts ):
                ...
                $text = _n( '%s ' . $post_type->labels->singular_name, '%s ' . $post_type->labels->name, $published, 'prosports' );
        ...
        <ul class="sp_status_list">
            ...
            if ( $next_event ):
                ...
                <li class="countdown" data-countdown="<?php echo str_replace( '-', '/', $next_event->post_date ); ?>">
                    <a href="<?php echo get_edit_post_link( $next_event->ID ); ?>">
                        <?php printf( __( '<strong>%s</strong> until next event', 'prosports' ), $interval->days . ' ' . __( 'days', 'prosports' ) . ' ' . sprintf( '%02s:%02s:%02s', $interval->h, $interval->i, $interval->s ) ); ?>
            ...
            <li class="events-scheduled">
                <a href="<?php echo admin_url( 'edit.php?post_type=sp_event&post_status=future' ); ?>">
                    <?php printf( _n( '<strong>%s event</strong> scheduled', '<strong>%s events</strong> scheduled', $scheduled_count, 'prosports' ), $scheduled_count ); ?>
                </a>
            </li>
            <li class="events-published">
                <a href="<?php echo admin_url( 'edit.php?post_type=sp_event&post_status=publish' ); ?>">
                    <?php printf( _n( '<strong>%s event</strong> published', '<strong>%s events</strong> published', $published_count, 'prosports' ), $published_count ); ?>
                </a>
            </li>
        </ul>
        <?php
