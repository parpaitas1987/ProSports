...
class SP_Meta_Box_Player_Statistics {
	...
	public static function output( $post ) {
		...
		?>
		<p><strong><?php _e( 'Career Total', 'prosports' ); ?></strong></p>
		<?php
				...
				<tbody>
					...
					foreach ( $data as $div_id => $div_stats ):
						...
						<tr class="sp-row sp-post<?php if ( $i % 2 == 0 ) echo ' alternate'; ?>">
							<td>
								<?php
								if ( 'WP_Error' == get_class( $div ) ) _e( 'Total', 'prosports' );
								else echo $div->name;
								?>
							</td>
							<?php if ( $league_id ): ?>
								...
								<?php else: ?>
									<td>
										...
										$args = array(
											...
											'show_option_none' => __( '&mdash; None &mdash;', 'prosports' ),
											'show_option_all' => __( '&mdash; Individual &mdash;', 'prosports' ),
										    'sort_order'   => 'ASC',
