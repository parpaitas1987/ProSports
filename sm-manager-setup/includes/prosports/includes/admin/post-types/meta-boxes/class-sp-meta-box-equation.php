...
class SP_Meta_Box_Equation {
	...
	public static function builder( $title = 'f(x)', $equation = '', $groups = array() ) {
		...
		foreach ( $groups as $group ):
			switch ( $group ):
				case 'player_event':
					$options[ 'Events' ] = array( '$eventsattended' => __( 'Attended', 'prosports' ), '$eventsplayed' => __( 'Played', 'prosports' ), '$eventsstarted' => __( 'Started', 'prosports' ), '$eventssubbed' => __( 'Substituted', 'prosports' ), '$eventminutes' => __( 'Minutes', 'prosports' ) );
					break;
				case 'team_event':
					$options[ 'Events' ] = array( '$eventsplayed' => __( 'Played', 'prosports' ), '$eventminutes' => __( 'Minutes', 'prosports' ) );
					break;
				case 'result':
					$options[ 'Results' ] = self::optgroup( 'sp_result', array( 'for' => '(' . __( 'for', 'prosports' ) . ')', 'against' => '(' . __( 'against', 'prosports' ) . ')' ), null, false );
					break;
				case 'outcome':
					$options[ 'Outcomes' ] = self::optgroup( 'sp_outcome' );
					$options[ 'Outcomes' ]['$streak'] = __( 'Streak', 'prosports' );
					$options[ 'Outcomes' ]['$last5'] = __( 'Last 5', 'prosports' );
					$options[ 'Outcomes' ]['$last10'] = __( 'Last 10', 'prosports' );
					break;
				case 'performance':
		...
		$options = apply_filters( 'prosports_equation_options', $options );
		?>
	...
	public static function equation_part_labels() {
		__( 'Operators', 'prosports' );
		__( 'Constants', 'prosports' );
	}
}