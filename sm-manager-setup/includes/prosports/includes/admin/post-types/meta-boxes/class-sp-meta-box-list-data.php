...
class SP_Meta_Box_List_Data {
	...
	public static function table( $columns = array(), $data = array(), $placeholders = array(), $adjustments = array(), $orderby = 'number' ) {
		$show_player_photo = get_option( 'prosports_list_show_photos', 'no' ) == 'yes' ? true : false;
		?>
				...
				<thead>
					<tr>
						<th><label for="sp_columns_number">
							...
							<?php echo 'number' == $orderby ? '#' : __( 'Rank', 'prosports' ); ?>
						</label></th>
						<th><?php _e( 'Player', 'prosports' ); ?></th>
				...
				<tbody>
					...
					if ( is_array( $data ) && sizeof( $data ) > 0 ):
						...
						foreach ( $data as $player_id => $player_stats ):
							...
							<tr class="sp-row sp-post<?php if ( $i % 2 == 0 ) echo ' alternate'; ?>">
								...
								<td>
									<?php if ( $show_player_photo ) echo get_the_post_thumbnail( $player_id, 'prosports-fit-mini' ); ?>
					...
					<tr class="sp-row alternate">
						<td colspan="<?php $colspan = sizeof( $columns ) + 3; echo $colspan; ?>">
							<?php printf( __( 'Select %s', 'prosports' ), __( 'Players', 'prosports' ) ); ?>
						</td>
					</tr>
					<?php
					endif;
