...
class SP_Admin_CPT {
	...
	function change_insert_into_post( $strings ) {
		...
		if ( $post_type == $this->type ) {
			...
			$strings['insertIntoPost']     = sprintf( __( 'Insert into %s', 'prosports' ), $obj->labels->singular_name );
			$strings['uploadedToThisPost'] = sprintf( __( 'Uploaded to this %s', 'prosports' ), $obj->labels->singular_name );
		}
