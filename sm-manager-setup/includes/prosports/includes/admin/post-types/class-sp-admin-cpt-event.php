...
class SP_Admin_CPT_Event extends SP_Admin_CPT {
	...
	public function edit_columns( $existing_columns ) {
		...
		$columns = array_merge( array(
			...
			'sp_format' => '<span class="dashicons sp-icon-calendar tips" title="' . __( 'Format', 'prosports' ) . '"></span>',
			'title' => null,
		...
		return apply_filters( 'prosports_event_admin_columns', $columns );
	}
		...
		switch ( $column ):
			...
			case 'sp_team':
				...
				if ( empty( $teams ) ):
					...
					echo '</div>';
					?>
					<div class="row-actions sp-row-actions"><span class="inline hide-if-no-js"><a href="#" class="sp-edit-results"><?php _e( 'Edit Results', 'prosports' ); ?></a></span></div>
	...
	public function filters() {
		...
		$args = array(
			'post_type' => 'sp_team',
			'name' => 'team',
			'show_option_none' => __( 'Show all teams', 'prosports' ),
			'selected' => $selected,
			'values' => 'ID',
		);
		...
		$args = array(
			'show_option_all' =>  __( 'Show all competitions', 'prosports' ),
			'taxonomy' => 'sp_league',
			'name' => 'sp_league',
			'selected' => $selected
		);
