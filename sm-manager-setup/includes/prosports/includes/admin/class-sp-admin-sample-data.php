<?php
/**
 * ProSports Admin Sample Data Class.
 *
 * The ProSports admin sample data class stores demo content.
 *
...
class SP_Admin_Sample_Data {
	...
	public static function insert_posts() {
		...
		// Competitions
		$taxonomies['sp_league'] = array( _x( 'Primary League', 'example', 'prosports' ), _x( 'Secondary League', 'example', 'prosports' ) );
		...
		// Jobs
		$taxonomies['sp_role'] = array( _X( 'Coach', 'example', 'prosports' ) );
		...
		$sample_content = _x( 'This is an example %1$s. As a new ProSports user, you should go to <a href=\"%3$s\">your dashboard</a> to delete this %1$s and create new %2$s for your content. Have fun!', 'example', 'prosports' );
		...
		foreach ( $teams as $index => $team ) {
			...
			$post['post_content'] = sprintf( $sample_content, __( 'Team', 'prosports' ), __( 'Teams', 'prosports' ), admin_url() );
		...
		foreach ( $players as $index => $name ) {
			...
			$post['post_status'] = 'publish';
			$post['post_content'] = sprintf( $sample_content, __( 'Player', 'prosports' ), __( 'Players', 'prosports' ), admin_url() );
		...
		foreach ( $staff as $index => $name ) {
			...
			$post['post_status'] = 'publish';
			$post['post_content'] = sprintf( $sample_content, __( 'Staff', 'prosports' ), __( 'Staff', 'prosports' ), admin_url() );
		...
		for ( $index = 0; $index < 6; $index ++ ) {
			...
			$post = array(
				'post_title' => $teams[ $i ]['name'] . ' ' . get_option( 'prosports_event_teams_delimiter', 'vs' ) . ' ' . $teams[ $away_index ]['name'],
				'post_type' => 'sp_event',
				'post_status' => $post_status,
				'post_content' => sprintf( $sample_content, __( 'Event', 'prosports' ), __( 'Events', 'prosports' ), admin_url() ),
		 ...
		 */
		$post = array(
			'post_title' => _x( 'Fixtures & Results', 'example', 'prosports' ),
			'post_type' => 'sp_calendar',
			'post_status' => 'publish',
			'post_content' => sprintf( $sample_content, __( 'Calendar', 'prosports' ), __( 'Calendars', 'prosports' ), admin_url() )
		);
		...
		$post = array(
			...
			'post_type' => 'sp_table',
			'post_status' => 'publish',
			'post_content' => sprintf( $sample_content, __( 'League Table', 'prosports' ), __( 'League Tables', 'prosports' ), admin_url() ),
			'tax_input' => array(
		...
		foreach ( $inserted_ids['sp_team'] as $index => $team_id ) {
			$post = array(
				'post_title' => get_the_title( $team_id ) . ' ' . _x( 'Roster', 'example', 'prosports' ),
				'post_type' => 'sp_list',
				'post_status' => 'publish',
				'post_content' => sprintf( $sample_content, __( 'Player List', 'prosports' ), __( 'Player Lists', 'prosports' ), admin_url() ),
			);
		 ...
		 */
		$post = array(
			'post_title' => _x( 'Player Ranking', 'example', 'prosports' ),
			'post_type' => 'sp_list',
			'post_status' => 'publish',
			'post_content' => sprintf( $sample_content, __( 'Player List', 'prosports' ), __( 'Player Lists', 'prosports' ), admin_url() ),
		);
