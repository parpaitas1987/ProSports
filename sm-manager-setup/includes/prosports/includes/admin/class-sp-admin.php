...
class SP_Admin {
	...
	public function __construct() {
		...
		add_action( 'prosports_settings_page', 'sp_review_link' );
		add_action( 'prosports_config_page', 'sp_review_link' );
		add_action( 'prosports_overview_page', 'sp_review_link' );
	}
...
//		include_once( 'sp-meta-box-functions.php' );
		...
		if ( ! is_ajax() ) {
			...
			if ( get_option( 'prosports_rich_editing', true ) ):
				include( 'class-sp-admin-editor.php' );
			endif;

			// Help
//			if ( apply_filters( 'prosports_enable_admin_help_tab', true ) )
...
//				include( 'class-sp-admin-profile.php' );
	...
	public function prevent_admin_access() {
		...
		if ( 'yes' == get_option( 'prosports_lock_down_admin' ) && ! is_ajax() && ! ( current_user_can( 'edit_posts' ) || current_user_can( 'manage_prosports' ) ) && basename( $_SERVER["SCRIPT_FILENAME"] ) !== 'admin-post.php' ) {
			$prevent_access = true;
		}

		$prevent_access = apply_filters( 'prosports_prevent_admin_access', $prevent_access );
