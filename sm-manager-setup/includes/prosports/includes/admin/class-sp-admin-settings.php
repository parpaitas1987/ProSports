<?php
/**
 * ProSports Admin Settings Class.
 *
 * @author 		ProSports
 * @category 	Admin
...
class SP_Admin_Settings {
	...
	public static function save() {
		...
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'prosports-settings' ) )
	    		die( __( 'Action failed. Please refresh the page and retry.', 'prosports' ) );

	    // Trigger actions
	   	do_action( 'prosports_settings_save_' . $current_tab );
	    do_action( 'prosports_update_options_' . $current_tab );
	    do_action( 'prosports_update_options' );

		self::add_message( __( 'Your settings have been saved.', 'prosports' ) );

		do_action( 'prosports_settings_saved' );
	}
	 ...
	 * Settings page.
	 *
	 * Handles the display of the main prosports settings page in admin.
	 *
	 * @access public
	 * @return void
	 */
	public static function output() {
	    ...
	    do_action( 'prosports_settings_start' );

	    wp_enqueue_script( 'prosports_settings', SP()->plugin_url() . '/assets/js/admin/settings.js', array( 'jquery', 'wp-color-picker', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'iris', 'chosen' ), SP()->version, true );

		wp_localize_script( 'prosports_settings', 'localized_strings', array(
	    ...
	    $tabs = apply_filters( 'prosports_settings_tabs_array', array() );
	                    ...
	                    <td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
	                    	<select
	                    		...
	                    		foreach ( $value['options'] as $group => $options ) {
	                    			?>
	                    			<optgroup label="<?php _e( $group, 'prosports' ); ?>">
	                    				<?php
	                    ...
	                    <td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							...
							<p>
								<label>
									...
									<?php _e( 'Install demo content', 'prosports' ); ?>
								</label>
							</p>
	                    </td>
	                </tr><?php
	    ...
	    foreach ( $options as $value ) {
	    	...
	    	switch ( $type ) {
		    	...
		    	default :
		    		...
		    		do_action( 'prosports_update_option_' . $type, $value );
	    	...
	    	// Custom handling
	    	do_action( 'prosports_update_option', $value );
	    }
