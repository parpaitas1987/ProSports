...
class SP_Widget_Event_List extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_sp_event_list', 'description' => __( 'A list of events.', 'prosports' ) );
		parent::__construct('sp_event_list', __( 'Event List', 'prosports' ) . ' (' . __( 'ProSports', 'prosports' ) . ')', $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		// Action to hook into
		do_action( 'prosports_before_widget_template', $args, $instance, 'event-list' );
		...
		// Action to hook into
		do_action( 'prosports_after_widget_template', $args, $instance, 'event-list' );
	...
	function update( $new_instance, $old_instance ) {
		...
		// Filter to hook into
		$instance = apply_filters( 'prosports_widget_update', $instance, $new_instance, $old_instance, 'event-list' );
	...
	function form( $instance ) {
		...
		// Action to hook into
		do_action( 'prosports_before_widget_template_form', $this, $instance, 'event-list' );
		?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'prosports' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo $this->get_field_id('id'); ?>"><?php printf( __( 'Select %s:', 'prosports' ), __( 'Calendar', 'prosports' ) ); ?></label>
		<?php
		$args = array(
		...
		<p><label for="<?php echo $this->get_field_id('status'); ?>"><?php _e( 'Status:', 'prosports' ); ?></label>
			<?php
			$args = array(
		...
		<div class="sp-date-selector">
			<p><label for="<?php echo $this->get_field_id('date'); ?>"><?php _e( 'Date:', 'prosports' ); ?></label>
				<?php
				$args = array(
					'show_option_default' => __( 'Default', 'prosports' ),
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'Number of events to show:', 'prosports' ); ?></label>
		...
		<select name="<?php echo $this->get_field_name('order'); ?>" id="<?php echo $this->get_field_id('order'); ?>" class="sp-select-order widefat">
			<option value="default" <?php selected( 'default', $order ); ?>><?php _e( 'Default', 'prosports' ); ?></option>
		...
		<p class="sp-prefs">
			...
			$the_columns = array(
				...
				'time' => __( 'Time/Results', 'prosports' ),
		...
		<label for="<?php echo $this->get_field_id('show_all_events_link'); ?>"><?php _e( 'Display link to view all events', 'prosports' ); ?></label></p>

		<?php
		// Action to hook into
		do_action( 'prosports_after_widget_template_form', $this, $instance, 'event-list' );
	}
}
