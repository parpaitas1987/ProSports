...
class SP_Widget_League_Table extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_league_table widget_sp_league_table', 'description' => __( 'Display a league table.', 'prosports' ) );
		parent::__construct('sp_league_table', __( 'League Table', 'prosports' ) . ' (' . __( 'ProSports', 'prosports' ) . ')', $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		// Action to hook into
		do_action( 'prosports_before_widget_template', $args, $instance, 'league-table' );
		...
		// Action to hook into
		do_action( 'prosports_after_widget_template', $args, $instance, 'league-table' );
	...
	function update( $new_instance, $old_instance ) {
		...
		// Filter to hook into
		$instance = apply_filters( 'prosports_widget_update', $instance, $new_instance, $old_instance, 'league-table' );

		return $instance;
	}

	function form( $instance ) {
		$defaults = apply_filters( 'prosports_widget_defaults', array( 'title' => '', 'id' => '', 'number' => 5, 'columns' => null, 'show_team_logo' => false, 'show_full_table_link' => true ) );
		...
		// Action to hook into
		do_action( 'prosports_before_widget_template_form', $this, $instance, 'league-table' );
		?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'prosports' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo $this->get_field_id('id'); ?>"><?php printf( __( 'Select %s:', 'prosports' ), __( 'League Table', 'prosports' ) ); ?></label>
		<?php
		$args = array(
			'post_type' => 'sp_table',
			'name' => $this->get_field_name('id'),
			'id' => $this->get_field_id('id'),
			'show_option_none' => __( '&mdash; Select &mdash;', 'prosports' ),
			'selected' => $id,
			'values' => 'ID',
			'class' => 'widefat',
		);
		if ( ! sp_dropdown_pages( $args ) ):
			sp_post_adder( 'sp_table', __( 'Add New', 'prosports' ) );
		endif;
		?>
		</p>

		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'Number of teams to show:', 'prosports' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id('show_team_logo'); ?>"><?php _e( 'Display logos', 'prosports' ); ?></label><br>
		...
		<label for="<?php echo $this->get_field_id('show_full_table_link'); ?>"><?php _e( 'Display link to view full table', 'prosports' ); ?></label></p>

		<?php
		// Action to hook into
		do_action( 'prosports_after_widget_template_form', $this, $instance, 'league-table' );
	}
}
