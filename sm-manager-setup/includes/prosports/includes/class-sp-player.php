 ...
 * Player Class
 *
 * The ProSports player class handles individual player data.
 *
 * @class 		SP_Player
 * @version		1.5
...
class SP_Player extends SP_Custom_Post {
	...
	public function data( $league_id, $admin = false ) {
		...
		$div_ids[] = 0;
		$season_names[0] = __( 'Total', 'prosports' );
		...
		foreach ( $div_ids as $div_id ):
			...
			foreach( $events as $event ):
				...
				if ( $minutes === '' ) $minutes = get_option( 'prosports_event_minutes', 90 );
		...
		foreach( $placeholders as $season_id => $season_data ):
			...
			if ( $team_id ):
				...
				if ( get_option( 'prosports_link_teams', 'no' ) == 'yes' ? true : false ):
		...
		if ( $admin ):
			...
			endforeach;
			$labels = array( 'name' => __( 'Season', 'prosports' ) );
			if ( in_array( 'team', $this->columns ) )
				$labels['team'] = __( 'Team', 'prosports' );
