<?php
/*
Plugin Name: ProSports Multisite
Plugin URI: http://prosportspro.com/
Description: Add multisite network support to ProSports.
Author: ProSports
...
/**
 * Main ProSports Multisite Class
 *
 * @class ProSports_Multisite
 * @version	1.4
 */
class ProSports_Multisite {
	...
	public function __construct() {
		...
		if ( is_multisite() ):
			...
			// Widgets
			add_filter( 'prosports_widget_update', array( $this, 'multisite_widget_update' ), 10, 4 );
			add_filter( 'prosports_widget_defaults', array( $this, 'multisite_widget_defaults' ) );
			add_action( 'prosports_before_widget_template', array( $this, 'multisite_before_widget'), 10, 3 );
			add_action( 'prosports_after_widget_template', array( $this, 'multisite_after_widget'), 10, 3 );
			add_action( 'prosports_before_widget_template_form', array( $this, 'multisite_before_widget_form' ), 10, 3 );
			add_action( 'prosports_after_widget_template_form', array( $this, 'multisite_after_widget_form' ), 10, 3 );
		endif;
	}
	...
	*/
	...
	function multisite_before_widget_form( $object, $instance, $widget = 'default' ) {
		?>
		<p><label for="<?php echo $object->get_field_id('site_id'); ?>"><?php printf( __( 'Site: %s', 'prosports' ), '' ); ?></label>
