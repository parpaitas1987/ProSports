...
class SP_Tournament_Meta_Boxes {
	...
	public function __construct() {
		...
		add_action( 'prosports_process_sp_tournament_meta', array( $this, 'save' ), 10, 2 );
	}
			...
			<thead>
				<tr>
					<?php for ( $round = 0; $round < $rounds; $round++ ): ?>
						<th>
							<input type="text" class="widefat" name="sp_labels[]" value="<?php echo sp_array_value( $labels, $round, '' ); ?>" placeholder="<?php printf( __( 'Round %s', 'prosports' ), $round + 1 ); ?>">
						</th>
					<?php endfor; ?>
				</tr>
			</thead>
