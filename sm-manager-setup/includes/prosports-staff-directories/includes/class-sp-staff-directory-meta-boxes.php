...
class SP_Staff_Directory_Meta_Boxes {
	...
	public function __construct() {
		...
		add_action( 'prosports_process_sp_directory_meta', array( $this, 'save' ), 10, 2 );
		add_action( 'prosports_process_sp_staff_meta', array( $this, 'staff_save' ), 15, 2 );
		add_action( 'prosports_process_sp_team_meta', array( $this, 'team_save' ), 15, 2 );
	}
	...
	public function add_meta_boxes() {
		...
		add_meta_box( 'sp_shortcodediv', __( 'Shortcode', 'prosports' ), array( $this, 'shortcode' ), 'sp_directory', 'side', 'default' );
		add_meta_box( 'sp_formatdiv', __( 'Layout', 'prosports' ), array( $this, 'format' ), 'sp_directory', 'side', 'default' );
		add_meta_box( 'sp_detailsdiv', __( 'Details', 'prosports' ), array( $this, 'details' ), 'sp_directory', 'side', 'default' );
		add_meta_box( 'sp_datadiv', __( 'Staff Directory', 'prosports' ), array( $this, 'data' ), 'sp_directory', 'normal', 'high' );
		add_meta_box( 'sp_editordiv', __( 'Description', 'prosports' ), array( $this, 'editor' ), 'sp_directory', 'normal', 'high' );

		// Add contact info meta box to staff
		add_meta_box( 'sp_contactdiv', __( 'Contact Info', 'prosports' ), array( $this, 'staff_contact' ), 'sp_staff', 'side', 'default' );

		// Add directories meta box to team
		if ( isset( $post ) && isset( $post->ID ) ):
			add_meta_box( 'sp_directoriesdiv', __( 'Staff Directories', 'prosports' ), array( $this, 'team_directories' ), 'sp_team', 'normal', 'high' );
		endif;
	}
	...
	public static function details( $post ) {
		...
		<div>
			...
			<p><strong><?php _e( 'Team', 'prosports' ); ?></strong></p>
			<p class="sp-tab-select">
				<?php
				$args = array(
					'post_type' => 'sp_team',
		...
		<div class="sp-data-table-container">
			<table class="widefat sp-data-table sp-staff-directory-table sp-sortable-table">
				<thead>
					<tr>
						<th class="icon">&nbsp;</th>
						<th><?php _e( 'Job', 'prosports' ); ?></th>
						<th><?php _e( 'Name', 'prosports' ); ?></th>
				...
				<tbody>
					...
					<tr class="sp-row alternate">
						<td colspan="<?php $colspan = sizeof( $labels ) + 4; echo $colspan; ?>">
							<?php
							if ( $data === null ) printf( __( 'Select %s', 'prosports' ), __( 'Details', 'prosports' ) );
							else _e( 'No results found.', 'prosports' );
							?>
						</td>
					</tr>
					<?php
	...
	public static function staff_contact( $post ) {
		...
		?>
		<p>
			<strong><?php _e( 'Phone', 'prosports' ); ?></strong>
			<?php if ( ! empty( $phone ) ): ?>
				<a class="sp-link" href="tel:<?php echo $phone; ?>" title="<?php _e( 'Phone', 'prosports' ); ?>"></a>
			<?php endif; ?>
		</p>
		<p><input type="text" id="sp_phone" name="sp_phone" value="<?php echo $phone; ?>"></p>

		<p>
			<strong><?php _e( 'Email', 'prosports' ); ?></strong>
			<?php if ( ! empty( $email ) ): ?>
				<a class="sp-link" href="mailto:<?php echo $email; ?>" title="<?php _e( 'Email', 'prosports' ); ?>"></a>
			<?php endif; ?>
		</p>
	...
	public static function team_directories( $post ) {
		...
		if ( $pagenow != 'post-new.php' ):
			...
			printf( __( 'No results found.', 'prosports' ) );
				...
				<thead>
					<tr>
						...
						<th class="column-directory">
							<?php _e( 'Directory', 'prosports' ); ?>
						</th>
						<th class="column-league">
							<?php _e( 'Competition', 'prosports' ); ?>
						</th>
						<th class="column-season">
							<?php _e( 'Season', 'prosports' ); ?>
						</th>
						<th class="column-layout">
							<?php _e( 'Layout', 'prosports' ); ?>
						</th>
					</tr>
				</thead>
				<tbody>
						...
						if ( sizeof( $data ) > 0 ):
							...
							foreach ( $data as $directory ):
								...
								<tr class="sp-row sp-post<?php if ( $i % 2 == 0 ) echo ' alternate'; ?>">
									...
									<td><?php echo get_the_terms ( $directory->ID, 'sp_league' ) ? the_terms( $directory->ID, 'sp_league' ) : __( 'All', 'prosports' ); ?></td>
									<td><?php echo get_the_terms ( $directory->ID, 'sp_season' ) ? the_terms( $directory->ID, 'sp_season' ) : __( 'All', 'prosports' ); ?></td>
					...
					<tr class="sp-row alternate">
						<td colspan="6">
							<?php printf( __( 'Select %s', 'prosports' ), __( 'Details', 'prosports' ) ); ?>
						</td>
					</tr>
					<?php
					endif;
