<?php
/*
Plugin Name: ProSports Branding
Plugin URI: http://prosportspro.com/
Description: White label ProSports branding.
Author: ProSports
...
/**
 * Main ProSports Branding Class
 *
 * @class ProSports_Branding
 * @version	1.4
 */
class ProSports_Branding {
	...
	public function __construct() {
		...
		add_filter( 'prosports_get_settings_pages', array( $this, 'add_settings_page' ) );
		...
		remove_action( 'prosports_settings_page', 'sp_review_link' );
		remove_action( 'prosports_config_page', 'sp_review_link' );
		remove_action( 'prosports_overview_page', 'sp_review_link' );
	}
	...
	*/
	...
	public function init() {
		// Get label
		$this->label = get_option( 'prosports_branding_label' );
	}
	...
	public function gettext( $translated_text, $untranslated_text, $domain ) {
		if ( $domain == 'prosports' && ! empty( $this->label ) && strpos( $translated_text, 'ProSports' ) !== false ):
			$translated_text = str_replace( 'ProSports', $this->label, $translated_text );
		endif;
	...
	public function rename_color_scheme() {
		...
		$enabled = get_option( 'prosports_enable_branding_css', 'no' );
		if ( $enabled !== 'yes' ) return $_wp_admin_css_colors;

		$colors = get_option( 'prosports_branding_css_colors' );
	...
	public function admin_enqueue_scripts() {
		...
		if ( in_array( $screen->id, array( 'toplevel_page_prosports' ) ) ) {
			wp_enqueue_style( 'prosports-branding-admin', SP_BRANDING_URL . 'css/admin.css', array(), SP_BRANDING_VERSION );
			wp_enqueue_script( 'prosports-branding-admin', SP_BRANDING_URL . 'js/admin.js', array( 'jquery' ), SP_BRANDING_VERSION );
			wp_enqueue_media();
	...
	public function admin_styles() {
		$icon = get_option( 'prosports_branding_icon' );
		if ( $icon ):
			$src = wp_get_attachment_image_src( $icon, 'prosports-fit-mini' );
			if ( $src ):
				?>
				<style type="text/css">
					#adminmenu #toplevel_page_prosports .toplevel_page_prosports div.wp-menu-image:before {
						content: '';
		...
		$colors = get_option( 'prosports_branding_css_colors', true );
		if ( ! $colors ) return;
	...
	public function login_logo() {
		$icon = get_option( 'prosports_branding_icon' );
		$src = wp_get_attachment_image_src( $icon, 'prosports-fit-icon' );
